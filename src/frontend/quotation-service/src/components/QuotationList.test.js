...
jest.mock('../services/quotationService', () => {
  ...
  // Export the mock as default to match how it's imported in component
  return { 
...
// This should be done before importing the component, but we need to check what's actually exported
describe('QuotationList Component', () => {
  ...
  test('should render empty state when no quotations exist', async () => {
    // Arrange
    const mockService = require('../services/quotationService').quotationService;
    mockService.getQuotations.mockResolvedValue([]);
  ...
  test('should render list of quotations when data exists', async () => {
    ...
    mockService.getQuotations.mockResolvedValue(mockQuotations);
  ...
  test('should display loading state while fetching quotations', async () => {
    ...
    mockService.getQuotations.mockReturnValue(promise);
  ...
  test('should handle error state when fetching fails', async () => {
    // Arrange
    const mockService = require('../services/quotationService').quotationService;
    mockService.getQuotations.mockRejectedValue(new Error('Network error'));
  ...
  test('should create new quotation successfully', async () => {
    ...
    mockService.getQuotations.mockResolvedValue([]);
    mockService.createQuotation = jest.fn().mockResolvedValue(createdQuotation);
    ...
    // Assert
    await waitFor(() => {
      expect(mockService.createQuotation).toHaveBeenCalledWith(
